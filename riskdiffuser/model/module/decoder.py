...
from riskdiffuser.model.diffusion_utils.sampling import dpm_sampler
from riskdiffuser.model.diffusion_utils.sde import SDE, VPSDE_linear
from riskdiffuser.utils.normalizer import ObservationNormalizer, StateNormalizer
from riskdiffuser.model.module.mixer import MixerBlock
from riskdiffuser.model.module.dit import TimestepEmbedder, DiTBlock, FinalLayer,RiskTimeAdapter
...
class DiT(nn.Module):
    def __init__(self, sde: SDE, route_encoder: nn.Module, depth, output_dim, hidden_dim=192, heads=6, dropout=0.1, mlp_ratio=4.0, model_type="x_start"):
        ...
        ######
        self.risk_adapter = RiskTimeAdapter(hidden_dim, time_gamma=0.5)  ##想强调高噪声阶段 gamma = 2.0
        ######
    ...
    def forward(self, x, t, cross_c, route_lanes, neighbor_current_mask, rho):
        """
        Forward pass of DiT.
        ...
        # Risk Dropout
        # if self.training:
        #     mask = (torch.rand_like(rho) > 0.02).float()
        #     rho = rho * mask

        #risk_emb = self.risk_adapter(rho, t)

        #rho_center  = 2 * rho - 1
        #rho_in = torch.sign(rho_center) * torch.sqrt(torch.clamp(torch.abs(rho_center), min=0.0))
        # risk_emb = self.risk_adapter(rho_in, t)
        #risk_emb = self.risk_adapter(rho, t)

        # risk_scale = 2.0
        # t_emb = t_emb + risk_scale * risk_emb 

        gamma_out, beta_out = self.risk_adapter(rho, t)
        t_emb = t_emb * (1.0 + gamma_out) + beta_out
